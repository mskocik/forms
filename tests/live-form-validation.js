 ...
 * TODO: move validator extension to separate file
 * TODO: rewrite in proper style
 * 
 * + Updated 'filled' validator line 859
 * 
 * 
 ...
 * - Nette.validators.BaseComponentsFormCustomValidator_onlyValidPhoneNumber
 */
	...
	*/
	...
	Nette.addError = function (elem, message) {
			...
			if (elem.focus && !LiveForm.getFormProperty(elem.form, "hasError")) {
				console.log('here FOCUSing', elem);
					if (!LiveForm.focusing) {
							LiveForm.focusing = true;
							if (elem.tagName === 'SELECT' && elem.closest('el-svelecte')) {
								elem.closest('el-svelecte').focus();
							} else if (elem.tagName === 'HIDDEN') {
								const richText = elem.parentElement.querySelector('rich-editor');
	...
	Nette.validators = {
			filled: function (elem, arg, val) {
					...
					if (Array.isArray(val)) return val.filter(e => e).length > 0;
			...
			BaseComponentsFormCustomValidator_fullUrl: function (elem, arg, val) {
				if (!val) {
					return false
				}
			...
			BaseComponentsFormCustomValidator_onlyValidPhoneNumber: function (elem, arg, val) {
				const customElement = elem.closest('phone-number');
				return customElement ? customElement.iti.isValidNumber() : true;
			}
	};
